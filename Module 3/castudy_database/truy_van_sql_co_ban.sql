use furama_database;
-- 2: Hiển Thị thông tin của tất cả nhân viên có trong hệ thống có tên bắt đầu là 1 trong các ký tự "H,T,K" và có tối đa 15 kí tự.
...
-- 20: Hiển thị thông tin của tất cả các nhân viên va khách hàng có trong hệ thống,thông tin hiển thị bao gồm
...
nhan_vien.dia_chi
from nhan_vien
union  select khach_hang.ma_khach_hang,
khach_hang.ho_ten,
khach_hang.email,
...
select hop_dong.ma_hop_dong from hop_dong where (month(hop_dong.ngay_lam_hop_dong) between 10 and 12) and year(hop_dong.ngay_lam_hop_dong)=2020)
and hop_dong.ma_hop_dong not in (select hop_dong.ma_hop_dong from hop_dong where (month(hop_dong.ngay_lam_hop_dong) between 1 and 6) and year(hop_dong.ngay_lam_hop_dong)=2021)
group by hop_dong.ma_hop_dong;
-- 13:Hiển thị thông tin các Dịch vụ đi kèm được sử dụng nhiều nhất bởi các Khách hàng đã đặt phòng. 
-- (Lưu ý là có thể có nhiều dịch vụ có số lần sử dụng nhiều như nhau).
select dich_vu_di_kem.ma_dich_vu_di_kem,
dich_vu_di_kem.ten_dich_vu_di_kem,
sum(ifnull(hop_dong_chi_tiet.so_luong,0)) as total
from dich_vu_di_kem
join hop_dong_chi_tiet on hop_dong_chi_tiet.ma_dich_vu_di_kem = dich_vu_di_kem.ma_dich_vu_di_kem
